...
class ChildPartTablePopup(QDialog):
    ...
    def insert_row(self, row_idx, name, algo, id_=None):
        ...
        edit_button.setFixedSize(150, 28)
    ...
    def load_data(self):
        try:
            ...
            for row_idx, (id_, name, number, date, vname, vcode, part_id) in enumerate(rows):
                ...
                self.table.setItem(row_idx, 1, QTableWidgetItem(str(number)))
                self.table.setItem(row_idx, 2, QTableWidgetItem(str(date)))
                self.table.setItem(row_idx, 3, QTableWidgetItem(vname))
                self.table.setItem(row_idx, 4, QTableWidgetItem(vcode))
                self.table.setItem(row_idx, 5, QTableWidgetItem(part_id))

                # Edit Button
                # Auto-detect correct icon path
                if getattr(sys, 'frozen', False):
                    base_path = sys._MEIPASS
                else:
                    base_path = os.path.abspath(".")
...
class BaumerCamThread(QThread):
    ...
    def __init__(self, serial=None, mode="auto", jumbo=False, mono=True, parent=None):
        super().__init__(parent)
        self.serial = serial
...
class AddminScreen(QWidget):
    ...
    def initUI(self):
        ...
        self.parameters_layout.setContentsMargins(30, 30, 30, 30)
        self.parameters_layout.setSpacing(20)

        self.parameters_frame = QFrame()
        self.parameters_frame.setStyleSheet("background-color: #e6f2ff; border: 1px solid #4d4d4d; border-radius: 10px; padding: 8px;")
        self.parameters_frame.setLayout(self.parameters_layout)
        self.parameters_frame.setFixedWidth(650)
        ...
        self.capture_image_button.setStyleSheet(input_style)
        self.capture_image_button.clicked.connect(self.capture_image)

        self.view_parent_button = QPushButton("View parent Details")
        self.view_parent_button.setStyleSheet(input_style)
        self.view_parent_button.clicked.connect(self.view_parent_details_popup)


        self.create_bounding_box_button = QPushButton("Create Bounding Box")
        self.create_bounding_box_button.setStyleSheet(input_style)
        self.create_bounding_box_button.clicked.connect(self.enable_bounding_box)

        self.view_child_parts_button = QPushButton("View Child Part Table")
        self.view_child_parts_button.setStyleSheet(input_style)
        self.view_child_parts_button.clicked.connect(self.open_child_part_table)

        self.save_details_button = QPushButton("Save Details")
        self.save_details_button.setStyleSheet(input_style)
        self.save_details_button.clicked.connect(self.save_details)

        self.save_child_button = QPushButton("Save Child Details")
        self.save_child_button.setStyleSheet(input_style)
        ...
        top_button_layout.addWidget(self.save_details_button)
        top_button_layout.addStretch(1)

        middle_button_layout = QHBoxLayout()
        middle_button_layout.addWidget(self.capture_image_button)
        middle_button_layout.addWidget(self.view_parent_button)

        bottom_button_layout = QHBoxLayout()
        bottom_button_layout.addWidget(self.create_bounding_box_button)
        bottom_button_layout.addWidget(self.view_child_parts_button)

        # 🔽 New layout for Save Child Details button (centered)
        save_child_center_layout = QHBoxLayout()
        save_child_center_layout.addStretch(1)
        save_child_center_layout.addWidget(self.save_child_button)
        save_child_center_layout.addStretch(1)
        ...
        self.main_layout.addWidget(self.parameters_frame, stretch=1)

        # --- Right Panel with Image Viewer and Buttons INSIDE Border ---
        self.image_frame = QFrame()
        self.image_frame.setStyleSheet("background-color: #f5f5f5; border: 1px solid #4d4d4d;")
        self.image_frame.setFixedSize(1180, 850)
        ...
        button_style = "background-color: #0078d7; color: white; font-weight: bold; border-radius: 5px;"
        ...
        self.image_view.setFixedSize(1140, 780)  # Fit inside the border
