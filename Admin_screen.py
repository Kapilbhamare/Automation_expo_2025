...
# from CameraViewer import CameraViewer  # ⬅️ At the top
...
class ResizableBoundingBox(QGraphicsRectItem):
    def __init__(self, rect: QRectF):
        ...
        self.handles = {}
        self.updating_handles = False  # ✅ Add this line here to prevent AttributeError
        self.initHandles()
    ...
    def updateHandles(self):
        ...
        self.updating_handles = True  # Prevent recursion
        ...
        self.updating_handles = False  # ✅ Reset guard


class ChildPartTablePopup(QDialog):
    """Popup to show child part data in a clean table with working buttons."""
    ...
    def __init__(self, parent=None, parent_part_name=None, is_memory_mode=True):
        ...
        self.parent = parent
        self.parent_part_name = parent_part_name  # ✅ Store parent part name
    ...
    def insert_row(self, row_idx, name, algo, id_=None):
        ...
        # Auto-detect correct resource path for PyInstaller EXE and Development
        ...
        button_style = """
            ...
            QPushButton:pressed {
                background-color: #0d47a1;  /* Even darker blue on press */
                border-style: inset;
                color: #eeeeee;
            }
        ...
        # delete_button.setFixedSize(150, 28)
    ...
    def delete_row(self, row_idx):
        ...
        if confirm == QMessageBox.Yes:
            try:
                ...
                conn.commit()
                print(f"Deleted child part '{name}' (ID: {id_})")
                self.reload_table()
            except Exception as e:
                print("Error deleting child part:", e)
...
class BoundingBoxDrawer(QGraphicsRectItem):
    def __init__(self, x, y, width, height, parent_widget=None):
        ...
        self.setZValue(1)  # Make sure it appears above the image
    ...
    def rotate_by_degree(self, angle):
        """Rotate the bounding box from the top-left corner."""
        self.setTransformOriginPoint(self.rect().topLeft())  # Set pivot
        self.setRotation(self.rotation() + angle)  # Incremental rotation
...
class BoundingBoxPopup(QDialog):
    def __init__(self, parent=None, row_index=None, existing_id=None, existing_name=None, existing_algo=None, rect_item=None):
        ...
        # Detect base path for resources (works for EXE and Dev)
        if getattr(sys, 'frozen', False):
            base_path = sys._MEIPASS
        else:
            base_path = os.path.abspath(".")

        down_arrow_icon_path = os.path.join(base_path, "resources", "down_arrow.png").replace("\\", "/")  # Windows-safe
    ...
    def crop_and_save(image, bbox_coords, save_directory, image_name, padding=5):
        """
        Crops the input image using a perspective transformation based on
        the provided bounding box coordinates and saves the output image.
        
        Parameters:
            image (numpy.ndarray): Input image (as read by cv2.imread).
            bbox_coords (list or np.array): List/array of four (x, y) points
                representing the bounding box corners in the following order:
                top-left, top-right, bottom-right, bottom-left.
            save_directory (str): Path to the folder where the cropped image
                will be saved.
            image_name (str): Base name for the cropped image file (without extension).
            padding (int): Amount of padding to add around the bounding box.
        
        Returns:
            warped (numpy.ndarray): The cropped (and possibly rotated) image.
            save_path (str): Full path where the cropped image is saved.
        """
        # Convert coordinates to a NumPy array of type float32
        pts = np.array(bbox_coords, dtype="float32")
        
        # Compute the width of the new image by taking the maximum distance
        ...
        # Add the specified padding to both dimensions
        padded_width = maxWidth + 2 * padding
        padded_height = maxHeight + 2 * padding
        
        # Define destination points (with padding at the borders)
        dst = np.array([
            [padding, padding],
        ...
        # Get the perspective transform matrix and apply the warp
        M = cv2.getPerspectiveTransform(pts, dst)
        warped = cv2.warpPerspective(image, M, (padded_width, padded_height))
        
        # Rotate if needed – for example, if height is greater than width
        ...
        # Save the resulting image (you can append additional info if needed)
    ...
    def save_details(self):
        ...
        # ➤ EDIT MODE (just update text fields)
        if self.existing_id:
            try:
                conn = pymysql.connect(** db_config)
                cursor = conn.cursor()
        ...
        # ➤ ADD MODE (requires bounding box)
        ...
        scene_corners = [
            self.rect_item.mapToScene(rect.topLeft()),
            self.rect_item.mapToScene(rect.topRight()),
            self.rect_item.mapToScene(rect.bottomRight()),
            self.rect_item.mapToScene(rect.bottomLeft())
        ]
        ...
        # Debug: Log the raw coordinates from the scene (for reference)
        print("Raw image corner coordinates (from scene):")
        for idx, p in enumerate(image_corners):
            print(f"Corner {idx}: (x: {p.x()}, y: {p.y()})")

        # Log displayed pixmap dimensions
        pixmap_width = pixmap_item.pixmap().width()
        pixmap_height = pixmap_item.pixmap().height()
        print("Displayed pixmap dimensions: width =", pixmap_width, ", height =", pixmap_height)

        # Log original image shape (from cv2 image)
        print("Original image shape (height, width, channels):", img.shape)

        # # Compute scale factors: compare real image size with displayed pixmap size
        # scale_x = img.shape[1] / pixmap_width
        # scale_y = img.shape[0] / pixmap_height
        # print("Scale factors: scale_x =", scale_x, ", scale_y =", scale_y)

        # Convert scene coordinates to image-space coordinates using scale factors.
        bbox_coords = [[int(p.x()), int(p.y())] for p in image_corners]


        # Log the scaled bounding box coordinates with labels to verify order.
        corner_order_labels = ["top-left", "top-right", "bottom-right", "bottom-left"]
        for label, coord in zip(corner_order_labels, bbox_coords):
            print(f"Corner {label}: {coord}")
        parent_name = self.parent.parent_part_name_input.text().strip()
        folder_path = os.path.join(os.getcwd(), parent_name)
        os.makedirs(folder_path, exist_ok=True)
        image_base_name = f"{new_name}_{new_algo}"  # Customize as needed
...
class EditParentPartPopup(QDialog):
    def __init__(self, parent=None, record=None, refresh_callback=None):
        ...
        # Detect base path for resources (handles EXE & Dev)
        if getattr(sys, 'frozen', False):
            base_path = sys._MEIPASS
        else:
            base_path = os.path.abspath(".")
...
class ParentPartTablePopup(QDialog):
    def __init__(self, parent=None):
        ...
        self.setFixedSize(1500, 800)  # Extended width for new columns
        self.setSizePolicy(QSizePolicy.MinimumExpanding, QSizePolicy.Preferred)
        ...
        """)
        header.setSectionResizeMode(QHeaderView.Interactive)  # ⬅️ Allows manual widths

        # 🧹 Remove this line entirely
        # header.setMinimumSectionSize(180)

        # ✅ Set precise column widths (icons stay narrow)
        self.table.setColumnWidth(0, 250)  # Parent Part Name
        self.table.setColumnWidth(1, 200)  # Parent Part Number
        self.table.setColumnWidth(2, 200)  # Date
        self.table.setColumnWidth(3, 180)  # Vendor Name
        self.table.setColumnWidth(4, 180)  # Vendor Code
        self.table.setColumnWidth(5, 190)  # Part ID
        self.table.setColumnWidth(6, 120)   # Edit
        self.table.setColumnWidth(7, 120)   # Delete
...
""")
    ...
    def load_data(self):
        try:
            ...
            # optional: keep a copy if you need it elsewhere
            self._rows = rows
            ...
            for row_idx, (id_, name, number, date, vname, vcode, part_id) in enumerate(rows):
                ...
                # Resolve icon paths (works for both EXE and dev)
    ...
    def delete_entry(self, id_):
        ...
        if confirm == QMessageBox.Yes:
            try:
                ...
                # 🔍 Fetch folder path before deletion
...
class ImageGraphicsView(QGraphicsView):
    """Custom QGraphicsView to support bounding box drawing."""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.parent = parent
        self.setScene(QGraphicsScene())
        self.setStyleSheet("background-color: white;")  # ✅ ensures clean white view
        self.drawing = False
        self.start_pos = None
        self.rect_item = None
    ...
    def mouseMoveEvent(self, event):
        if self.drawing and self.rect_item:
            ...
            # ✅ Call updateHandles only if it exists and handles are initialized
    ...
    def mouseReleaseEvent(self, event):
        if self.bounding_box_enabled and self.rect_item:
            self.drawing = False
            self.start_pos = None
            if not self.rect_item.rect().isEmpty():  # ✅ Only set if box is valid
                self.last_drawn_box = self.rect_item
                print("[✅ ImageGraphicsView] Box Set:", self.last_drawn_box.rect())
            else:
                self.scene().removeItem(self.rect_item)  # Remove empty box visually
                self.rect_item = None
                print("[⚠️ Skipped empty box]")
        else:
            super().mouseReleaseEvent(event)


    def delete_last_rectangle(self):
        for item in self.scene().items():  # First item is most recently added
    ...
    def keyPressEvent(self, event):
        ...
        if self.interaction_mode == "rotate":
            if key == Qt.Key_E:
                self.rotate_selected_boxes(1)  # Clockwise
            elif key == Qt.Key_R:
                self.rotate_selected_boxes(-1)  # Anticlockwise
            else:
...
class BaumerCamThread(QThread):
    """Grab a single frame from a Baumer GigE (neoAPI) camera in the background."""
    frame_captured = pyqtSignal(object)   # emits a NumPy array
    ...
    def run(self):
        try:
            # Lazy import (optional): keeps module import cheap if no camera present
            import neoapi
            ...
            try:
                ...
                if self.mode == "auto":
                    ...
                    # warm-up a few frames so auto settles
                    for _ in range(8):
                        try:
...
class AddminScreen(QWidget):
    ...
    def __init__(self):
        ...
        self.camera_thread = None  # Optional: make sure thread ref is initialized

        self.initUI()

    def toggle_sidebar(self):
        """Toggle the visibility of the left sidebar (parameters_frame)."""
    ...
    def initUI(self):
        ...
        # 🟢 Dynamically size the window based on screen resolution
        screen_geometry = QApplication.desktop().screenGeometry()
        available_width = screen_geometry.width()
        available_height = screen_geometry.height()
        # self.setFixedSize(int(available_width * 0.9), int(available_height * 0.9))
        self.resize(1400, 1000)  # or an appropriate fixed size
        ...
        # Add vertical spacing between form and button grid
...
""")
        ...
        self.parameters_frame.setMinimumWidth(0)
        self.parameters_frame.setMaximumWidth(480)  # or 450 depending on needs
        self.parameters_frame.setMinimumWidth(430)
        self.parameters_frame.setMinimumHeight(650)  # Increase form height
        self.parameters_frame.setMaximumHeight(850)  # Optional

  # or 350, as needed

        # Detect base path for resources (EXE + Dev mode)
        if getattr(sys, 'frozen', False):
            base_path = sys._MEIPASS
        else:
            base_path = os.path.abspath(".")
        ...
        down_arrow_icon_path = os.path.join(base_path, "resources", "down_arrow.png").replace("\\", "/")  # Windows-friendly
        calendar_icon_path = os.path.join(base_path, "resources", "calendar1.png").replace("\\", "/")
        ...
        # Apply the calendar input style
        self.date_input.setStyleSheet(calendar_input_style)
        # Override just the input appearance
        self.date_input.setStyleSheet(calendar_input_style + """
        QDateEdit {
            padding: 5px;
            font-size: 15px;
        ...
        self.date_input.calendarWidget().setStyleSheet(calendar_input_style)  # 👈 ADD THIS LINE
        ...
        self.save_child_button.setIcon(qta.icon("fa5s.save", color="white"))  # Reuse save icon
...
# Set consistent size
        for btn in [
            self.save_details_button,
            self.capture_image_button,
            self.view_parent_button,
            self.view_child_parts_button,
            self.create_bounding_box_button,
            self.save_child_button
]:
            btn.setIconSize(QSize(30, 30))       # Smaller icon
...
""")
            ...
            # btn.setToolButtonStyle(Qt.ToolButtonTextUnderIcon)  # Icon above text
...
# 📌 2 Rows × 3 Columns Button Grid
        ...
        # Create a plain widget instead of a QGroupBox to avoid border & title
        button_box = QWidget()
        ...
        # Add the grid layout to the parameters layout
            # self.parameters_layout.addLayout(button_grid)

        # Add layouts in order
        # self.parameters_layout.addLayout(top_button_layout)
        # self.parameters_layout.addLayout(middle_button_layout)
        # self.parameters_layout.addLayout(bottom_button_layout)
        # self.parameters_layout.addLayout(save_child_center_layout)
        ...
        self.left_layout.setContentsMargins(0, 0, 0, 0)  # Adds side padding
        self.main_layout.addWidget(self.left_container)
        self.main_layout.setSpacing(20)  # add horizontal gap between form and image viewer


        # --- Right Panel with Image Viewer and Buttons INSIDE Border ---
...
""")
        ...
        # self.right_box.setMinimumWidth(700)
        # self.right_box.setMaximumWidth(int(available_width * 0.6))
        ...
        # Add/Edit/Delete Buttons at Top of Image Frame
        button_row = QHBoxLayout()
        button_row.setContentsMargins(0, 0, 0, 0)  # 🔧 Remove top margin
        button_row.setSpacing(8)  # 🔧 Tighten spacing between buttons
        ...
        self.add_button.setStyleSheet(icon_button_style)
        self.edit_button.setStyleSheet(icon_button_style)
        self.delete_button.setStyleSheet(icon_button_style)
        self.rotate_button.setStyleSheet(icon_button_style)
        self.add_button.setCheckable(True)
        self.edit_button.setCheckable(True)
        self.delete_button.setCheckable(True)
        self.rotate_button.setCheckable(True)
        # 🔗 Connect buttons to update their active state
        ...
        # top_button_style = self.get_top_button_style()

        # for btn in [self.add_button, self.edit_button, self.delete_button, self.rotate_button]:
        #     btn.setFixedSize(40, 30)
        #     btn.setStyleSheet(top_button_style)
        ...
        self.main_layout.setStretch(0, 0)  # left panel
        self.main_layout.setStretch(1, 2)  # right panel
    ...
    def show_placeholder_image(self):
        # Defer actual rendering to ensure correct dimensions
        QTimer.singleShot(0, self._render_placeholder)

    def _render_placeholder(self):
        self.image_scene.clear()

        # Use same fixed size as in capture_image()
        target_width = 1190
        target_height = 830

        # Set fixed-size scene rect
        self.image_scene.setSceneRect(0, 0, target_width, target_height)

        # Add a fixed black border rectangle
    ...
    def on_add_clicked(self):
        print("[👉 on_add_clicked] Checking last_drawn_box...")  # Add this
        last_box = self.image_view.last_drawn_box
        print("[👀 last_box]", last_box)  # Debug what it is
    ...
    def on_rotate_clicked(self):
        ...
        self.image_view.setFocus()  # Make sure key events go to view
    ...
    #     self.parameters_layout.addLayout(row_layout)
    ...
    def add_parameter_to_layout(self, label_text, widget):
        ...
        if isinstance(widget, QDateEdit):
            widget.setCalendarPopup(True)  # Required for the icon to show up


        form_item_layout = QVBoxLayout()
        form_item_layout.setSpacing(8)  # ⬅️ Add more space between label and field
        form_item_layout.setContentsMargins(0, 0, 0, 15)  # ⬅️ Add space below each field group
    ...
    def capture_image(self):
        # Optional: choose mode="auto" for auto exposure/gain; "manual" for fixed values
        self.camera_thread = BaumerCamThread(
            serial=None,      # set your camera serial string if you want to target a specific device
            mode="auto",
            jumbo=False,
            mono=False,
            parent=self
    ...
    def display_captured_image(self, frame):
        ...
        self.image_view.setScene(self.image_scene)
        self.image_view.setSceneRect(QRectF(pixmap.rect()))  # ✅ FIXED

        # Store full-resolution pixmap
        self.image_view.current_pixmap = pixmap
        self.image_view.pixmap_item = pixmap_item

        print(f"[✅ Camera Frame] Displayed frame resolution: {h}x{w}")
    ...
    def display_captured_image(self, frame):
        ...
        # ➤ Show the warped image instead of original
        ...
        print(f"[✅ Warped Frame] Displayed warped resolution: {h}x{w}")
    ...
    def enable_bounding_box(self):
        ...
        self.image_view.last_drawn_box = None  # Reset
    ...
    def save_child_details(self):
        ...
        try:
            # Step 1: Extract image from QPixmap
            ...
            # Step 3: Parse and structure coordinates with vision_algorithm prefix
            parsed_coords = []
            for row in coords_rows:
                coord_str = row[0]
                vision_algorithm = row[1]  # Adjust index if necessary
                ...
                try:
                    ...
                    print(f"✅ Parsed Coordinates: {formatted}")

                except Exception as e:
                    print(f"❌ Parsing Error for row: {coord_str}\n{e}")
                    continue  # Skip this entry instead of breaking the entire loop


            # Step 4: Save Crops using master function
            save_dir = os.path.join(os.getcwd(), parent_name)
            save_master_crop(img, parsed_coords, save_dir)

            QMessageBox.information(self, "Success", "Child parts cropped and saved successfully.")

        except Exception as e:
            print("❌ Error in save_child_details():", e)
            QMessageBox.critical(self, "Cropping Error", f"Failed to crop child parts:\n{str(e)}")
    ...
    def save_details(self):
        ...
        try:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO parent_part (parent_part_name, parent_part_number, created_date, folder_path, part_identification_no, vendor_name, vendor_code) VALUES (%s, %s, %s, %s, %s, %s, %s)",
                (parent_name, parent_number, date, folder_path, part_id, vendor_name, vendor_code)
            )
            conn.commit()
            print(f"✅ Parent part saved: {parent_name}")
            QMessageBox.information(self, "Success", f"Parent part '{parent_name}' saved successfully!")
        except Exception as e:
